...
if (process.env.FIREBASE_PRIVATE_KEY) {
  ...
  // Fix badly formatted keys (missing header/footer from copy-paste)
  ...
  // Fallback to AI Studio provisioned config
  let projectId = "maximal-carving-wf6jr";
  try {
    ...
    if (error.code === 'auth/user-not-found') {
      try {
        ...
        if (createError.code === 'auth/configuration-not-found' || createError.message.includes('CONFIGURATION_NOT_FOUND')) {
          console.warn('\n--- WARNING ---');
          console.warn('Firebase Authentication is not enabled for this project.');
          console.warn('Please go to the Firebase Console -> Authentication -> Sign-in method and enable Email/Password.');
          console.warn('-----------------\n');
        } else {
          console.error('Error creating default admin user (ignoring so server can start):', createError.message);
        }
      }
      ...
      console.error('Error checking for default admin user (ignoring so server can start):', error.message);
    }
  }
}
...
async function startServer() {
  ...
  // Seed default demo hotels if local file does not exist
  if (!fs.existsSync(localStorePath)) {
    ...
    fs.writeFileSync(localStorePath, JSON.stringify(initialHotels, null, 2), 'utf-8');
  }
  ...
  if (!fs.existsSync(scanLogsPath)) {
    fs.writeFileSync(scanLogsPath, JSON.stringify([], null, 2), 'utf-8');
  }
  ...
  // API endpoint for client components / SPA scan redirect
  ...
  app.get('/scan/:qrId', async (req, res, next) => {
    ...
    // Direct 302 HTTP Redirect to the Google Maps/Review URL
