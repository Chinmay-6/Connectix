    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 glass-panel rounded-2xl p-6 border border-slate-800 space-y-6">
          {error && (
            <div className="p-4 bg-red-950/40 border border-red-800/50 rounded-xl text-red-300 text-xs font-medium flex items-start gap-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-900/90 border border-slate-800 rounded-xl px-4 py-3.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-medium transition-all"
                required
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500 font-medium transition-all cursor-pointer"
              >
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-4 px-6 rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center space-x-2 disabled:opacity-50 uppercase text-xs tracking-wider cursor-pointer"
              >
        ...
        <div className="lg:col-span-5 glass-card rounded-2xl p-6 border border-slate-800 space-y-6">
          ...
          <div className="flex flex-col items-center justify-center bg-white p-6 rounded-2xl shadow-inner text-slate-900 space-y-3">
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(previewTarget)}`}
              alt="QR Preview"
              className="w-40 h-40 rounded-lg shadow-sm border border-slate-200"
            />
          ...
          <div className="p-3 bg-emerald-950/40 border border-emerald-800/40 rounded-2xl text-xs text-emerald-300 flex items-start gap-2.5">
            ...
            <div>
              ...
              <div className="text-[11px] text-emerald-300/90 mt-0.5">
                Encodes <code className="font-mono bg-emerald-900/60 px-1 rounded text-emerald-200">/scan/QR...</code>. When scanned, server logs scan event in Analytics and opens Google Maps!
              </div>
            </div>
          </div>
          ...
          {googleReviewUrl && (
            <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs font-mono text-blue-400 hover:underline truncate flex items-center gap-1.5"
              >
